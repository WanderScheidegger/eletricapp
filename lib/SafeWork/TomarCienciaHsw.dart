...
class TomarCienciaHsw extends StatefulWidget {
  @override
  _TomarCienciaHswState createState() => _TomarCienciaHswState();
}

class _TomarCienciaHswState extends State<TomarCienciaHsw> {
  ...
  //ALERT DIALOG
  static _displayDialog_Ciencia(context, item) async {
    return showDialog(
        context: context,
        builder: (context) {
          return AlertDialog(
            title: Text(
              "Tem certeza que deseja tomar ciência?",
  ...
  static _tomarCiencia(context, item){
    ...
    db.collection("OST")
        .document(item)
        .updateData({"ciencia" : "Sim", "data_ciencia" : _dataCiencia })
        .then((onValue){
      pr.hide();
